...
class TagApiTestCase(LiveServerTestCase):
  ...
  def test_tag_list(self):
    resp = self.client.get(self.live_server_url + "/api/v2/tags/" )
    self.assertEqual(resp.status_code, 200)
    data = resp.json()
    self.assertEqual(len(data), 4)
    self.assertEqual(self.tag_vales, {t["value"] for t in data})
  ...
  def test_tag_create_basic_auth(self):
    ...
    resp=self.client.post(
      self.live_server_url + "/api/v2/tags/",{"value":"tag5"}
      )
  ...
  def test_tag_create_token_auth(self):
    token_resp = self.client.post(
      self.live_server_url + "/api/v2/token-auth/",
